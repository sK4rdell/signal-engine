...
.PHONY: ingest
ingest: ## Ingest a source: make ingest [source=arbetsmiljoverket|stockholm] [feed=recurring-inspection-failures] from=2026-09-20 to=2026-09-23
	$(LOAD_ENV) go run ./cmd/ingest $(or $(source),arbetsmiljoverket) $(if $(feed),--feed $(feed)) $(if $(from),--from $(from)) $(if $(to),--to $(to))
